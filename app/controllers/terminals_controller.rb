class TerminalsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @terminal.save
        ...
        format.xml { render :show, status: :created, location: @terminal }
      else
        format.html { render :new }
        format.json { render json: @terminal.errors, status: :unprocessable_entity }
        format.xml { render json: @terminal.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def update
    respond_to do |format|
      if @terminal.update(terminal_params)
        ...
        format.xml { render :show, status: :ok, location: @terminal }
      else
  ...
  private
    ...
    def terminal_params
      params.require(:terminal).permit(:name, :description, :attrs)
    end
end
